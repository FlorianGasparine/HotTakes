...
exports.updateSauce = (req,res,next) => {
    ...
    SauceModel.updateOne({_id : req.params.id}, { ... sauceObjet, _id: req.params.id})
...
exports.getSauceWithId = (req,res,next) => {      // Le ':' dit a Express que c'est un parametre de route dynamique
    SauceModel.findOne({_id : req.params.id})
        .then((modelsSauceSchema => res.status(200).json(modelsSauceSchema)))
    ...
    switch (like) {
        ...
        case 1 :
            SauceModel.updateOne(
                {_id : sauceId},                        // Recupere l'id de la sauce dans la requeteb (présente dans les paramètres de l'url)
                {
        ...
        case -1 : 
            ...
            .then(()=> res.status(200).json({message : "L'utilisateur à dislike la sauce"}))
            .catch((error => res.status(400).json({error})))
            break;
